...
import { 
  Heart, Repeat, Share2, MessageCircle, Send, CheckCircle, Clock, Eye, 
  ChevronRight, Loader2, Star, Sparkles, Award, ChevronDown, ChevronUp
} from 'lucide-react';
...
const ObraDetalle = () => {
  ...
  const handleInspirar = async () => {
    ...
    try {
      await api.post(`/reactions/toggle`, { post_id: obra.id, reaction_type: 'inspire' });
  ...
  const handleGuardar = async () => {
    if (!token) return navigate('/login');
    try {
      const res = await api.post(`/saved-items/toggle`, { post_id: obra.id });
      const isSaved = res.data.status === 'added';
      setHasGuardado(isSaved); 
      showToast(isSaved ? t('artwork.toasts.saved', '⭐ Añadida a tu colección') : t('artwork.toasts.unsaved', '🗑️ Eliminada de la colección'));
    } catch (error) { showToast(t('artwork.toasts.sync_error', '❌ Error al sincronizar.')); }
  };
    ...
    if (navigator.share) {
      try { 
        await navigator.share({ 
          title: obra.title, 
          text: t('artwork.share_text', 'Explora el lienzo: {{title}}', { title: obra.title }), 
      ...
      try {
        ...
        showToast(t('artwork.toasts.link_copied', '🔗 Enlace copiado. ¡Listo para compartir!'));
        actionSuccessful = true;
      } catch (e) {
        showToast(t('artwork.toasts.copy_error', '❌ No se pudo copiar el enlace.'));
      }
    }
    if (actionSuccessful) {
      try {
        await api.post(`/posts/${obra.id}/share`);
        setObra(prev => ({ 
          ...prev, 
          stats: { 
            ...prev.stats, 
        ...
        showToast(t('artwork.toasts.share_success', '🔄 ¡Repost registrado con éxito!'));
      } catch (error) {
        console.error("Error al registrar el repost:", error);
        showToast(t('artwork.toasts.error_share', '❌ Error al registrar el repost.'));
      }
    }
  };
  ...
  const enviarComentario = async (e) => {
    ...
    setEnviando(true);
    try {
      const res = await api.post(`/comments`, { post_id: obra.id, content: nuevoComentario });
      setObra(prev => ({ ...prev, comments: [res.data.data, ...(prev.comments || [])] }));
      setNuevoComentario('');
      showToast(t('artwork.toasts.comment_added', '💬 Aporte registrado.'));
    } catch (error) { 
      console.error(error);
      showToast(t('artwork.toasts.comment_error', '❌ Error al comentar.')); 
            ...
            <div className="w-full lg:w-[40%] xl:w-[35%] flex flex-col gap-3 lg:sticky lg:top-20">
              ...
              <Link to={`/artesanos/${obra.author?.username || ''}`} className="w-full bg-[var(--bg-container)]/50 backdrop-blur-2xl rounded-[18px] p-3 border border-[var(--border-color)] shadow-lg flex items-center justify-between group hover:border-[rgb(var(--role-accent))]/40 transition-all duration-500">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[8px] font-bold text-[var(--text-body)] uppercase tracking-[0.2em] flex items-center gap-1"><Award size={8} className="text-[rgb(var(--role-accent))]"/> {t('artwork.author.role', 'Maestro')}</span>
                    <span className="text-xs font-bold text-[var(--text-heading)] tracking-tight group-hover:text-[rgb(var(--role-accent))] transition-colors">{obra.author?.name || t('artwork.author.anonymous', 'Anónimo')}</span>
                  </div>
                </div>
              ...
              <div className="flex gap-3 w-full">
                <button onClick={handleGuardar} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3.5 rounded-[16px] font-bold uppercase tracking-[0.2em] text-[9px] transition-all ${hasGuardado ? 'bg-[rgb(var(--role-accent))]/20 border border-[rgb(var(--role-accent))] text-[rgb(var(--role-accent))]' : 'bg-[var(--bg-container)]/50 backdrop-blur-md border border-[var(--border-color)] text-[var(--text-heading)] hover:bg-[var(--text-heading)]/10'}`}>
                    ...
                    {hasGuardado ? t('artwork.actions.saved', 'Guardada') : t('artwork.actions.save', 'Coleccionar')}
                </button>
                <button onClick={handleShareMaster} className="w-[45px] shrink-0 flex items-center justify-center rounded-[16px] bg-[var(--bg-container)]/50 backdrop-blur-md border border-[var(--border-color)] text-[var(--text-body)] hover:text-[var(--text-heading)] hover:bg-[var(--text-heading)]/10 transition-all" title={t('artwork.actions.share', 'Compartir Obra')}>
                    <Share2 size={14} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between px-5 py-4 rounded-[18px] bg-[var(--bg-container)]/30 backdrop-blur-md border border-[var(--border-color)] mb-10 lg:mb-0 transition-colors duration-500">
                <div className="flex flex-col items-center group cursor-pointer w-1/3 border-r border-[var(--border-color)]" onClick={handleInspirar}>
                  ...
                  <span className="text-[7px] font-medium tracking-[0.2em] text-[var(--text-body)] uppercase">{t('artwork.stats.inspiration', 'Inspiración')}</span>
                </div>
                <div className="flex flex-col items-center group cursor-pointer w-1/3 border-r border-[var(--border-color)]" onClick={handleShareMaster}>
                  ...
                  <span className="text-[7px] font-medium tracking-[0.2em] text-[var(--text-body)] uppercase" title="Clic para Compartir">{t('artwork.stats.repost', 'Repost')}</span>
                </div>
                <div className="flex flex-col items-center w-1/3">
                  ...
                  <span className="text-[7px] font-medium tracking-[0.2em] text-[var(--text-body)] uppercase">{t('artwork.stats.views', 'Vistas')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="w-full lg:w-[60%] xl:w-[65%] flex flex-col pt-0">
              <div className="mb-6">
                <span className="inline-flex items-center gap-1.5 bg-[rgb(var(--role-accent))]/10 border border-[rgb(var(--role-accent))]/20 text-[rgb(var(--role-accent))] px-3 py-1.5 rounded-full text-[8px] font-bold uppercase tracking-widest shadow-sm mb-4">
                  <Sparkles size={10} /> {obra.category?.name || t('artwork.category_fallback', 'Arte Local')}
                </span>
              ...
              <div className="w-full max-w-[400px] bg-[var(--bg-container)]/60 backdrop-blur-xl rounded-full p-1.5 flex items-center border border-[var(--border-color)] shadow-sm mb-5 transition-colors duration-500">
                <button onClick={() => setActiveTab('relato')} className={`flex-1 py-2.5 rounded-full text-[9px] font-bold uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'relato' ? 'bg-[rgb(var(--role-accent))] text-white shadow-[0_0_15px_rgba(var(--role-accent),0.3)]' : 'text-[var(--text-body)] hover:text-[var(--text-heading)]'}`}>
                  {t('artwork.tabs.story', 'Relato Visual')}
                </button>
                <button onClick={() => setActiveTab('comunidad')} className={`flex-1 py-2.5 rounded-full text-[9px] font-bold uppercase tracking-[0.2em] transition-all duration-300 ${activeTab === 'comunidad' ? 'bg-[rgb(var(--role-accent))] text-white shadow-[0_0_15px_rgba(var(--role-accent),0.3)]' : 'text-[var(--text-body)] hover:text-[var(--text-heading)]'}`}>
                  {t('artwork.tabs.community', 'Perspectivas')} ({obra.comments?.length || 0})
                </button>
              </div>
              ...
              <div className="w-full">
                {activeTab === 'relato' && (
                  <div className="animate-in fade-in duration-500">
                    <div className="text-[var(--text-body)] text-sm md:text-base leading-relaxed font-medium bg-[var(--bg-container)]/50 p-6 md:p-8 rounded-[24px] border border-[var(--border-color)] shadow-inner transition-colors duration-500">
                      {shouldTruncate ? (
                        <>
                          <div ref={contentRef} className={`whitespace-pre-wrap ${!expandedContent ? 'line-clamp-6' : ''}`}>
                            {obra.content || t('artwork.content_empty', 'Este registro visual no cuenta con un relato extendido adjunto.')}
                          </div>
                          <button 
                            onClick={toggleContent}
                            className="mt-4 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-[rgb(var(--role-accent))] hover:underline transition-colors"
                          >
                            {expandedContent ? (
                    ...
                    {token ? (
                      <form onSubmit={enviarComentario} className="relative shadow-sm">
                        <textarea 
                          ...
                          placeholder={t('artwork.comments.placeholder', 'Escribe tu interpretación...')}
                      ...
                      <div className="bg-[var(--bg-container)]/40 backdrop-blur-2xl p-6 md:p-8 rounded-[24px] border border-[var(--border-color)] flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm transition-colors duration-500">
                        <div className="flex items-center gap-4 text-left">
                          ...
                          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-body)]">{t('artwork.comments.login_prompt', 'Inicia sesión para comentar.')}</p>
                        </div>
                        <Link to="/login" className="bg-[var(--text-heading)] text-[var(--bg-primary)] hover:bg-[rgb(var(--role-accent))] hover:text-white px-8 py-3 rounded-full text-[9px] font-bold uppercase tracking-widest transition-all whitespace-nowrap shadow-sm">
                          {t('artwork.comments.login_button', 'Ingresar')}
                        </Link>
                      </div>
                    )}
                      ...
                      {obra.comments?.map((com) => (
                        <div key={com.id} className="bg-[var(--bg-container)]/30 backdrop-blur-xl border border-[var(--border-color)] hover:border-[rgb(var(--role-accent))]/20 rounded-[20px] p-5 flex flex-col gap-3 transition-colors">
                          <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-3">
                            ...
                            <div className="flex items-center gap-1.5 text-[8px] font-mono text-[var(--text-body)] uppercase tracking-wider">
                              <Clock size={10}/> {com.human_time || new Date(com.created_at).toLocaleDateString()}
                            </div>
                          </div>
